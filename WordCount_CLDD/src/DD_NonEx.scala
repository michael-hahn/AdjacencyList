...
class DD_NonEx[T] {
  ...
  def ddgen(inputRDD: RDD[T], lineageInfo: RDD[Any], testFunc: userTest[T], splitFunc: userSplit[T], lm: LogManager, fh: FileHandler): Unit = {
    dd_helper(inputRDD, lineageInfo, 2, testFunc, splitFunc, lm, fh)
  }
