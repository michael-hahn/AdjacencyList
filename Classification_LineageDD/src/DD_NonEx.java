...
public class DD_NonEx<T> {
    ...
    public boolean test(JavaRDD<T> inputRDD, userTest<T> testFunc, int maxClusters, int totalClusters, Cluster[] centroid_ref,LogManager lm, FileHandler fh) {
        return testFunc.usrTest(inputRDD, maxClusters, totalClusters, centroid_ref,lm, fh);
    }

    private void dd_helper(JavaRDD<T> inputRDD, int numberOfPartitions, userTest<T> testFunc, userSplit<T> splitFunc, int maxClusters, int totalClusters, Cluster[] centroid_ref,
        ...
        while (true) {
            ...
            boolean assertResult = test(rdd, testFunc, maxClusters, totalClusters, centroid_ref, lm, fh);
...
//            }
            ...
            for (int i = 0; i < partitions; i++) {
                ...
                boolean result = test(rddList[i], testFunc, maxClusters, totalClusters, centroid_ref, lm, fh);
            ...
            if (!rdd_failed) {
                for (int j = 0; j < partitions; j++) {
                    ...
                    boolean result = test(rddBar, testFunc, maxClusters, totalClusters, centroid_ref, lm, fh);
                    if (result) {
                        rddBar_failed = true;
    ...
    public void ddgen(JavaRDD<T> inputRDD, userTest<T> testFunc, userSplit<T> splitFunc, int maxClusters, int totalClusters, Cluster[] centroid_ref, LogManager lm, FileHandler fh) {
        dd_helper(inputRDD, 2, testFunc, splitFunc, maxClusters, totalClusters, centroid_ref, lm, fh);
    }
