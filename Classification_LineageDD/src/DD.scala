...
class DD[T] {
  ...
  def test(inputRDD: JavaRDD[T], testFunc: userTest[T], maxClusters: Int, totalClusters: Int, centroid_ref: Array[Cluster], lm: LogManager, fh: FileHandler): Boolean = testFunc.usrTest(inputRDD, maxClusters,
    totalClusters, centroid_ref, lm, fh)
  ...
  private def dd_helper(inputRDD: JavaRDD[T],
    ...
    while (!failing_stack.isEmpty) {
      breakable {
        ...
        val assertResult = test(rdd, testFunc, maxClusters, totalClusters, centroid_ref, lm, fh)
        if (!assertResult) {
        ...
        for (i <- 0 until partitions) {
          ...
          val result = test(rddList(i), testFunc, maxClusters, totalClusters, centroid_ref, lm, fh)
        ...
        if (!rdd_failed) {
          for (j <- 0 until partitions) {
            ...
            val result = test(rddBar, testFunc, maxClusters, totalClusters, centroid_ref, lm, fh)
            if (result) {
              rddBar_failed = true
  ...
  def ddgen(inputRDD: JavaRDD[T], testFunc: userTest[T], splitFunc: userSplit[T], maxClusters: Int, totalClusters: Int, centroid_ref: Array[Cluster], lm: LogManager, fh: FileHandler) {
